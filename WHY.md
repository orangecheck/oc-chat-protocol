...
The shipped v0 (SPEC §7.6) is concretely this: the body text is AES-GCM-locked under a reveal secret `R`; `R` is drand-quicknet-`tlock`'d to the round for the target block and rides *inside* the ciphertext (the postage precedent — `lock-core` can't commit a top-level seal field). To pull the Bitcoin leg as far as a browser honestly can, we add a **hard chain gate**: a conforming client refuses to surface the body until it independently verifies `chain_tip ≥ unlock_block + confirmations` against a named explorer, *even if `tlock` already yielded `R`*. So a message opens at the **later** of (the drand round elapsing, the chain reaching the height). This doesn't change the verdict — drand still *withholds* the key, so a colluding threshold reads early on a non-conforming client — but it makes the displayed promise ("readable ~at block N") true for a conforming client without pretending Bitcoin holds the key. The block is a *verified condition* (offline-checkable, invariant 5), never the lock.
